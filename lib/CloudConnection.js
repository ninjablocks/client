...
CloudConnection.prototype.dataHandler = function dataHandler(device) {
  ...
  return function(data) {
    ...
    try {
      ...
      self.log.debug('Error sending data (%s)', this.app.getGuid(device));
      self.log.error(e);
    }
  };
};
...
CloudConnection.prototype.config = function config(req) {
  ...
  this.log.debug('Got config request', req);
...
CloudConnection.prototype.sendConfig = function sendConfigReply(id, responses, sync) {
