      ...
      if (response.statusCode == 200) {
        if (body) {
          ...
          try {
            ...
            return deferred.reject(new Error('Failed parsing credentials from cloud : ' + body));
          }
...
CloudConnection.prototype.up = function up(cloud) {
  ...
  this.log.info('Client connected to ' + this.opts.apiHost);
  this.log.info('sample#mqtt.connect_count=' + this.reconnectCount + ' sample.node_uptime=' + process.uptime());
...
CloudConnection.prototype.error = function error(err) {
  ...
  if (err.message.match(/Bad username or password/)) {
    ...
    this.log.info("saving token.");

    // Attempt to save an empty token and restart
    this.creds.saveToken(this.creds, function(err){
      setTimeout(function backoff(){
        this.log.info("shutting down.");
        process.exit(0);
      }.bind(this), 3000);
    }.bind(this));
