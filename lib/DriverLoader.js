...
function DriverLoader(app, configPath) {
  ...
  driverPaths.forEach(function onEachDriverPath(path) {
    ...
    try {
      fs.readdirSync(path).forEach(function(driver) {
        try {
          ...
          self.log.warn('Failed to load driver:', driver, 'Error:', e);
        }
  ...
  app.on('config::request', function onConfigRequest(id, requests, sync) {
...
DriverLoader.prototype.loadDriver = function(name, path) {
  ...
  driver.on('register', function onDriverRegister(device) {
    driver.log.debug('Device registered. G:%s V:%s D:%s name:%s', device.G, device.V, device.D, device.name);
    this.app.emit('device::register', device, name);
  }.bind(this));

  driver.on('announcement', function onDriverAnnouncement(data) {
