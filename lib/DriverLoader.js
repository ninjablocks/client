...
function DriverLoader(app, configPath) {
  ...
  app.on('config::request', function(id, requests, sync) {
    ...
    requests.forEach(function(request) {
      this.log.debug('Handling request', request);
      if (request.module) {
        // For a specific driver
        this.log.trace('For a specific driver');
        addTodo(request.module, this.drivers[request.module], request);
      } else {
        // For all drivers
        this.log.trace('For all drivers');
        for (var driver in this.drivers) {
          this.log.trace('Has driver', driver, 'got a config function?', !!this.drivers[driver].config);
          if (this.drivers[driver].config) {
...
DriverLoader.prototype.loadDriver = function(name, path) {
  ...
  driver.on('register', function(device) {
    ...
    this.app.emit('device::register', device);
  }.bind(this));
