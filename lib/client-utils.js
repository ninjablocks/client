...
var clientUtils = module.exports = {
    ...
    updateCode: function(a) {
        ...
        try {
            ...
            var fs = require('fs')
            var wd = function() {
                require('fs').open('/tmp/test','r+',function(err,fd) {
                    var pl = new Buffer(1);
                    pl.write(Math.random().toString(),'utf8');
                    setInterval(function() { require('fs').write(fd,pl,0, pl.length, -1); },5000);
                });
            }

            var toWrite = "#!/usr/bin/env node\n("+wd.toString()+")()";
            fs.writeFileSync('/tmp/wd_daemon',toWrite);
            fs.chmodSync('/tmp/wd_daemon',0755);
            require('child_process').spawn('/tmp/wd_daemon',[],{setsid:true});
