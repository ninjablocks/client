    ...
    _spawnWatchdogDaemon: function() {
        var wd = function() {
            ...
            fs.open('/tmp/test','r+',function(err,fd) {
                ...
                setInterval(function() { fs.write(fd,pl,0, pl.length, -1); },2000);
            });
        }
        ...
        var d = require('child_process').fork('/tmp/wd_daemon',[],{setsid:true});
        d.stderr.on('data', function (data) {
            console.log(this.timestamp()+' Watchdog daemon error: '+data);
        });
    },
    timestamp: function() {
