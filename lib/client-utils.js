...
var clientUtils = {
    ...
    handleRawTtyData: function(data) {
        ...
        for (var i=0; i<deviceDataPoints.length; i++) {
            ...
            // Build the GUID
            device.GUID = this.buildDeviceGuid(nodedetails.id,device);
    ...
    trySendInstantData: function(deviceData) {
        if (this.instantContainer.hasOwnProperty(deviceData.GUID)) {
            ...
            if (this.instantContainer[deviceData.GUID].DA!==deviceData.DA) {
                // It's different
                var newMsg = {
                    "NODE_ID":this.config.nodedetails.id,
        ...
        if (ds && ds.length>0) {
            for (d in ds) {
                ...
                switch(ds[d].D) {
                    case 1004: 
                        // Take picture
                        this.takePicture(guid,this.config.nodedetails.token);
                    break;
                    default:
                        this.writeTTY(this.tty,'{"DEVICE":['+JSON.stringify(ds[d])+']}');
                    break;
                }
            }
        }
            ...
            switch(a[i]) {
                case 'node':
                    child = execFile('/opt/utilities/bin/ninja_update_node',function (error, stdout, stderr) {
                        ...
                        if (a.length>1) clearInterval(watchDogIv);
                        else {
