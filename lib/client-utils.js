...
var clientUtils = {
    ...
    configure: function(config,remote,tty) {
        this.config=config||{};
        this.remote=remote||{};
        this.tty=tty||{};
    },
    ...
    updateCode: function(a) {
        ...
        for (var i=0;i<a.length;i++) {
            ...
            switch(a[i]) {
                case 'node':
                    var child = execFile('/opt/utilities/bin/ninja_update_node',function (error, stdout, stderr) {
    ...
    fetchBlockToken: function() {
        return path.existsSync(this.config.tokenFile) && fs.readFileSync(this.config.tokenFile).toString().replace(/\n/g,'') || false;
    }
}
