...
var clientUtils = module.exports = {
    ...
    debounceDevice: function(device) {
        ...
        if (this.debounceContainer.hasOwnProperty(guid)) {
            ...
            if (lastSentDeviceData.TIMESTAMP+deviceMetaData.debounceTimeout > now) return;
    ...
    queueTTY: function(device) {
        ...
        var queueListener = function(deviceAck) {
            ...
            emitter.removeListener('DeviceAcknowledgement',queueListener);

            if (!util.isArray(deviceAck)) return;
            var guid = self.buildDeviceGuid(self.config.id,deviceAck[0]);

            if (!self.queueContainer.hasOwnProperty(guid)) {
                // No queue for this device, so ignore it
                return;
            }

            if (self.queueContainer[guid].length>0) {
                // Left pop the queue
                self.queueContainer[guid].splice(0,1);
            }

            if (self.queueContainer[guid].length>0) {
                // Write the next comand
                self.writeTTY(self.tty,'{"DEVICE":['+JSON.stringify(self.queueContainer[guid][0])+']}');
            }
        };

        var guid = self.buildDeviceGuid(self.config.id,device);

        this.queueContainer[guid] = this.queueContainer[guid] || [];

        var timeOut = setTimeout(function() {
            // Command timed out, continue with next one
            queueListener([device])
        },1000);

        emitter.on('DeviceAcknowledgement',queueListener);

        if (this.queueContainer[guid].length===0) {
            this.writeTTY(this.tty,'{"DEVICE":['+JSON.stringify(device)+']}');
        }

        this.queueContainer[guid].push(device);
        return true;
    },
